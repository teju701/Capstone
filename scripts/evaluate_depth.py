...
validation set at native 1024×2048 resolution. Prints a full metrics
report and saves it to disk along with sample visualisations.
...
def save_eval_sample(img_tensor, pred_tensor, target_tensor, path):
    ...
    plt.suptitle("Depth Estimation Baseline - Evaluation Sample (1024×2048)", fontsize=14)
    plt.tight_layout()
...
def main():
    ...
    with torch.no_grad():
        for i, batch in enumerate(tqdm(val_loader, desc="Evaluating (1024x2048)")):
    ...
    lines.append("-" * 58)
    lines.append(f"  delta < 1.25   : {avg['δ<1.25']:.4f}   (higher better, target > 0.85)")
    lines.append(f"  delta < 1.25^2 : {avg['δ<1.25²']:.4f}   (higher better, target > 0.95)")
